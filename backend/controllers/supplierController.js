...
const createSupplierProduct = asyncHandler(async (req, res) => {
  const { name, cropSelection, storage, image, phonenumber, description } =
    req.body;

  let email = `${crypto.randomBytes(20).toString("hex")}@gmail.com`;
  console.log(email);
  let address = "Lunawada";
  console.log(address);
